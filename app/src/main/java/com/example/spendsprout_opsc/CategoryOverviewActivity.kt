...
import com.SBMH.SpendSprout.categories.HierarchicalCategoryAdapter
import com.SBMH.SpendSprout.categories.CategoryViewModel
...
class CategoryOverviewActivity : AppCompatActivity() {
    ...
    private fun setupDrawer() {
        ...
        navView.setNavigationItemSelectedListener { item ->
            when (item.itemId) {
                R.id.nav_overview -> startActivity(Intent(this, com.SBMH.SpendSprout.overview.OverviewActivity::class.java))
                R.id.nav_categories -> { /* already here */ }
                R.id.nav_transactions -> startActivity(Intent(this, com.SBMH.SpendSprout.transactions.TransactionsActivity::class.java))
                R.id.nav_accounts -> startActivity(Intent(this, com.SBMH.SpendSprout.accounts.AccountsActivity::class.java))
                R.id.nav_reports -> Toast.makeText(this, "Reports coming soon!", Toast.LENGTH_SHORT).show()
                R.id.nav_settings -> startActivity(Intent(this, com.SBMH.SpendSprout.settings.SettingsActivity::class.java))
    ...
    private fun setupCategoryRecyclerView() {
        ...
        hierarchicalCategoryAdapter = HierarchicalCategoryAdapter(
            emptyList(),
            onCategoryClick = { category ->
                // Navigate to activity_categories (CategoriesActivity) for selected main category (Needs, Wants, Savings)
                val intent =
                    Intent(this, com.SBMH.SpendSprout.categories.CategoriesActivity::class.java)
                intent.putExtra("filterByCategory", category.name)
    ...
    private fun setupFab() {
        ...
        fabAddCategory.setOnClickListener {
            ...
            val intent = Intent(this, com.SBMH.SpendSprout.edit.EditCategoryActivity::class.java)
            startActivity(intent)
        }
    }

    private fun observeViewModel() {
        categoryViewModel.loadCategoriesWithSubcategories()
        categoryViewModel.categoriesWithSubcategories.observe(this) { categoriesWithSubcategories ->
    ...
    private fun updateDateRangeDisplay() {
        if (startDate != null && endDate != null) {
            ...
            txtDateRange.text = when {
                ...
                daysDifference < 7 -> "Last $daysDifference days"
                daysDifference < 30 -> "Last ${daysDifference / 7} weeks"
                daysDifference < 365 -> "Last ${daysDifference / 30} months"
                else -> "Last ${daysDifference / 365} years"
            }
        } else {
